...
export default function PanelsPage() {
  ...
  function resetForm() {
    setName(""); setUrl(""); setUsername(""); setPassword(""); setApiToken(""); setSubUri("");
  ...
  async function savePanel(e: FormEvent) {
    e.preventDefault();
    setSaving(true);
    const body = { name, url, username, password: password || undefined, apiToken: apiToken || undefined, subUri: subUri || undefined, active: true };
    const res = editId
      ...
      <div className="p-3 sm:p-6 max-w-4xl mx-auto">
        ...
        {showForm && (
          <form onSubmit={savePanel} className="p-6 rounded-lg shadow mb-6 space-y-4" style={{ background: "var(--bg-secondary)" }}>
            ...
            <input placeholder="آدرس (https://example.com:54321)" value={url} onChange={(e) => setUrl(e.target.value)}
              className="w-full p-3 rounded border outline-none" style={{ background: "var(--bg-tertiary)", color: "var(--text-primary)", borderColor: "var(--border-color)" }} dir="ltr" required />
            ...
            <div>
              <label className="text-xs text-gray-400">Subscription Base URL (اختیاری)</label>
            ...
            >
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-gray-400 text-sm truncate" dir="ltr">{panel.url}</p>
                  <div className="flex items-center gap-3 mt-1">
                    <p className="text-gray-500 text-xs whitespace-nowrap">{new Date(panel.createdAt).toLocaleDateString("fa-IR")}</p>
