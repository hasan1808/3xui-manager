...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/admins/me").then((r) => { if (r.ok) return r.json(); throw new Error(); }).catch(() => router.push("/login"));
      ...
      <div className="p-3 sm:p-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <div className="rounded-lg p-5 shadow transition-shadow duration-200 hover:shadow-xl" style={{ background: "var(--bg-secondary)" }}>
            ...
            <p className="text-green-400 text-xs">آنلاین من</p>
          </div>
          <div className="rounded-lg p-5 shadow transition-shadow duration-200 hover:shadow-xl" style={{ background: "var(--bg-secondary)" }}>
        ...
        {financeSummary && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-8">
            ...
            <div className="rounded-lg p-5 shadow transition-shadow duration-200 hover:shadow-xl" style={{ background: "var(--bg-secondary)" }}>
              <p className="text-gray-400 text-xs mb-1">شارژ/واریزی</p>
              <p className="text-2xl font-bold text-blue-400">{financeSummary.myDeposits.toLocaleString("en-US")}</p>
              <p className="text-gray-500 text-xs">{financeSummary.currency}</p>
            </div>
            <div className="rounded-lg p-5 shadow transition-shadow duration-200 hover:shadow-xl" style={{ background: "var(--bg-secondary)" }}>
            ...
            {financeSummary.isSuperadmin && (
              <div className="rounded-lg p-5 shadow transition-shadow duration-200 hover:shadow-xl cursor-pointer" style={{ background: "var(--bg-secondary)" }}
                ...
                <p className="text-gray-400 text-xs mb-1">پنل مدیریت</p>
                <p className="text-2xl font-bold text-purple-400">{financeSummary.adminCount}</p>
                <p className="text-purple-400 text-xs">ادمین‌ها</p>
              </div>
            )}
          </div>
        )}
