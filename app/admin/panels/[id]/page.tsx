...
export default function PanelDetail() {
  ...
  const host = panel?.clientUrl ? extractHost(panel.clientUrl) : (panel?.url ? extractHost(panel.url) : "");
      ...
      try {
        ...
        if (settingsRes.ok) {
          ...
          if (subUriVal) { s = `${subUriVal.replace(/\/+$/, "")}/${subId}`; }
            ...
            const h = extractHost(panel?.clientUrl || panel?.url || "");
            if (h) s = `${panel?.clientUrl?.startsWith("https") || panel?.url?.startsWith("https") ? "https" : "http"}://${h}:${subPortVal}${subPathVal}${subId}`;
          }
        }
      } catch {}
    }
    if (!s && panel?.clientUrl) { s = `${panel.clientUrl.replace(/\/+$/, "")}/${subId}`; }
    if (!s) { const h = extractHost(panel?.clientUrl || panel?.url || ""); if (h) s = `https://${h}:2096/sub/${subId}`; }
